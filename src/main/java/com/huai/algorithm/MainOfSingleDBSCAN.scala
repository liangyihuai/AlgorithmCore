...
import com.huai.algorithm.util.{DateUtils, ParamsUtils}
...
/**
  ...
  * ./bin/spark-submit --master spark://master:7077 --class com.huai.algorithm.MainOfSingleDBSCAN /home/liangyh/IdeaProjects/GraduateProject/out/artifacts/algorithm_jar/algorithm.jar  phoneIndex:13 timeIndex:3 longitudeIndex:29 latitudeIndex:30 boxLen:0.02 eps:0.06 minPoints:2 durationTime:7200000 subtractTimeOfAdjacentStatus:3600000 minPointNumOfACluster:4 inputPath:/home/liangyh/Documents/GRADUATE-PROJECT/DataSet/AGG_3G_BSSAP_UTF8-original.txt
  * Created by liangyh on 2/16/17.
  */
object MainOfSingleDBSCAN{

  private var paramsOfAlgorithm:ParamsOfAlgorithm = null;
  private var metadataOfDataset:MetadataOfDataset = null;
  ...
  def main(args:Array[String]):Unit = {
    ...
    out = new BufferedWriter( new OutputStreamWriter(new FileOutputStream("/home/liangyh/tmp/result.txt")))

    val params = ParamsUtils.parseParamsStr(args)
    metadataOfDataset = params._1
    paramsOfAlgorithm = params._2

    println(metadataOfDataset)
    println(paramsOfAlgorithm)
  ...
  def doBusiness(data:RDD[String]):Unit ={
    ...
    val validInfo:RDD[Data] = getValidInfo(
      data,
      metadataOfDataset.phoneIndex,
      -1,
      metadataOfDataset.timeIndex,
      metadataOfDataset.longitudeIndex,
      metadataOfDataset.latitudeIndex)
    ...
    val clusteringRDD = groupedRDD.map(item =>{
      ...
      filteredStatusArray.foreach(println)
      (item._1, filteredStatusArray)
    })

    if(clusteringRDD == null){
      logger.warn("++++++++++++++++++null+++++++++++++++++")
    }else{
      logger.warn("++++++++++++++++++not null+++++++++++++++++")
    }
    try{
      println(clusteringRDD.count()+"-----------------+++++++++++++++")
    }catch {
      case e:Exception => {
        e.printStackTrace()
    ...
    logger.warn("cluster with single")
//    clusterWithSingle(clusteringRDD)
  ...
  def clusterWithSingle(clusteringRDD: RDD[(String,ArrayBuffer[BaseStatus])]): Unit ={
    clusteringRDD.filter(item => item != null && item._2 != null).foreach{
      case item: (String, ArrayBuffer[BaseStatus]) => {
        val key = item._1
  ...
  /**
    ...
    * @param clusters
    */
  private def postClusterForSingle(phone:String, clusters :ArrayBuffer[Cluster[AttachmentPoint[BaseStatus]]]):Unit ={
    ...
    //过滤掉点数不够的簇
    val filteredClusters = clusters.filter(cluster => cluster != null && cluster.getPoints.size() >= paramsOfAlgorithm.minPointNumOfACluster)
    ...
    filteredClusters.foreach(cluster => {
      ...
      if(totalTime != 0){
        ...
        val result = s"$phone\t$finalXCoordinate\t$finalYCoordinate\t$beginTime\t$endTime\n"
        out.write(result)
      }
    })
  }
  ...
  def filterStatus(statusList:ArrayBuffer[BaseStatus]):ArrayBuffer[BaseStatus] ={
    ...
    groupedData.foreach(item => {
      ...
      tempArray
        .filter(v => {v != null && v.endTime>0 && DateUtils.subtract(v.startTime.toString, v.endTime.toString) >= paramsOfAlgorithm.durationTime})
